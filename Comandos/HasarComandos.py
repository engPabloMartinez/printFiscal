...
class HasarComandos(ComandoInterface):
    ...
    def openTicket(self, defaultLetter="B"):
        ...
        self._savedPayments = []
        self._currentDocumentType = defaultLetter
...
##  NO SE PUEDE
    ...
    def addItem(self, description, quantity, price, iva, tasaAjusteInternos, 
        ...
        reply = self._sendCommand(self.CMD_PRINT_LINE_ITEM,
                                   [self._formatText(description[-1], 'lineItem'),
                                     quantityStr, priceUnitStr, ivaStr, sign, tasaAjuste, "1", terminalItem])
        if discount:
            if discountNegative:
                sign = 'm'
            else:
    ...
    def addReturnRecharge(self, description, price, iva, tasaAjusteInternos, itemNegative=False):
        ...
        for d in description[:-1]:
            self._sendCommand(self.CMD_PRINT_TEXT_IN_FISCAL, [self._formatText(d, 'fiscalText'), "0"])
        reply = self._sendCommand(self.CMD_RETURNRECHARGE,
                                   [self._formatText(description[-1], 'lineItem'),
                                     priceUnitStr, ivaStr, sign, tasaAjuste, "1", terminalItem, "B"])        
        return reply	
    ...
    def addRemitItem(self, description, quantity):
        ...
        return self._sendCommand(self.CMD_PRINT_EMBARK_ITEM,
                                   [self._formatText(description, 'embarkItem'), quantityStr, "0"])
